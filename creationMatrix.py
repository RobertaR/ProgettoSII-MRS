...
def person_corr_similarity(x, y):
    num = 0
    den1 = 0
    den2 = 0
...
person_corr_similarity_matrix = np.zeros((n_users, n_users))
for i in range(0, n_users):
    for j in range(0, n_users):
        if i!=j:
            person_corr_similarity_matrix[i][j] = person_corr_similarity(i + 1, j + 1)
            sys.stdout.write("\rGenerating Similarity Matrix [%d:%d] = %f" % (i+1, j+1, person_corr_similarity_matrix[i][j]))
            sys.stdout.flush()
            time.sleep(0.00005)
print "\rGenerating Similarity Matrix [%d:%d] = %f" % (i+1, j+1, person_corr_similarity_matrix[i][j])

print person_corr_similarity_matrix
...
def guess(user_id, i_id, top_n):
    ...
    for i in range(0, n_users):
        if i+1 != user_id:
            similarity.append(person_corr_similarity_matrix[user_id-1][i])
    temp = norm()
